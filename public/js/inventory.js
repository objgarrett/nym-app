...
var mainFxn = () =>{
    ...
    $.ajax({
      ...
      $.ajax({
        ...
        $.ajax({
          type: "GET",
          url: "/api/house/" + house + "/inventory"
        }).done(apiInventory => {
          console.log(apiInventory);
          for (var j = 0; j < apiInventory.length; j++){
              var divToAdd = $(`<div class="div-to-append house-inventory"><div class="row" id="toggle-box"><label class="switch"><input type="checkbox"><span class="slider round"></span></label><div id="inventorycontainer"><div id="inventory">${apiInventory[j].inventorytext}</div>`)
