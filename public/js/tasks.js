        ...
        $.ajax({
          ...
          for (var j = 0; j < apiTasks.length; j++){
            ...
            if (parseInt(apiTasks[j].userid) === parseInt(facebook)) {
              ...
              var divToAdd = $(`<div class="div-to-append user-task"><div class="row"><div class="col-sm" id="tasks" ><div><img src="css/images/checked-white.png" alt="update button" class="check-button" name="${num}" onclick="checkTask(${taskNum})"></div>${apiTasks[j].text} ${word}</div><img src="css/images/garbage-2-white.png" alt="delete button" class="delete-button" onclick="deleteTask(${taskNum})"><i class="far fa-caret-square-down" class="more-button" onclick="showDiv(${num})"></i></div></div><div class="row"><div class="col-sm" id="taskcontainer${num}"><p id="task-edit">Update Chore:</p><input type="text" id="task-edit"><p id="assigned-to">Assigned to: ${person}</p><input type="text" id="assigned-to"><p id="due-by">Due On: ${date}</p><input type="date" id="due-by"></div></div></div>`)
