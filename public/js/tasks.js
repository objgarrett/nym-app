        ...
        $.ajax({
          ...
          for (var j = 0; j < apiTasks.length; j++){
            ...
            if (parseInt(apiTasks[j].userid) === parseInt(facebook)) {
              ...
              var divToAdd = $(`<div class="div-to-append user-task" facebook="${facebook}" taskid="${apiTasks[i].taskid}" id="${num}"><div class="row"><div class="col-sm" id="taskcontainer"><div><img src="css/images/edit-white.png" alt="update button" class="check-button" name="${num}"></div><div id="tasks">${apiTasks[j].text}</div><img src="css/images/edit-white.png" alt="update button" class="update-button"><img src="css/images/garbage-2-white.png" alt="delete button" class="delete-button"><i class="far fa-caret-square-down" class="more-button"></i></div></div><div class="row"><div class="col-sm"><p id="task-edit">Update Chore:</p><input type="text" id="task-edit"><p id="assigned-to">Assigned to: ${person}</p><input type="text" id="assigned-to"><p id="due-by">Due On: ${date}</p><input type="date" id="due-by"></div></div></div>`)
